...
CREATE TABLE returns (
    ...
    reason VARCHAR(255),
    repurchase_id VARCHAR(20),
    created_at DATETIME NOT NULL,
    FOREIGN KEY (order_id) REFERENCES orders(order_id) ON DELETE RESTRICT,
    FOREIGN KEY (repurchase_id) REFERENCES orders(order_id) ON DELETE SET NULL,
    INDEX idx_order_id (order_id),
    INDEX idx_status (status),
    INDEX idx_repurchase_id (repurchase_id),
    INDEX idx_created_at (created_at)
);
...
SELECT 
    ...
    r.reason,
    r.repurchase_id,
    rp.total_amount AS repurchase_amount,
    r.created_at,
    o.user_id,
    COUNT(rl.return_line_id) AS items_returned
FROM returns r
JOIN orders o ON r.order_id = o.order_id
LEFT JOIN orders rp ON r.repurchase_id = rp.order_id
LEFT JOIN return_lines rl ON r.return_id = (
    SELECT return_id FROM returns WHERE order_id = r.order_id LIMIT 1
)
GROUP BY r.return_id, r.order_id, r.return_amount, r.status, r.reason, r.repurchase_id, rp.total_amount, r.created_at, o.user_id;
