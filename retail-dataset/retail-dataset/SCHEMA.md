...
| repurchase_id | VARCHAR(20) | FK → orders | Order created from return (exchange or promoted new order) |
| created_at | DATETIME | NOT NULL | Return creation timestamp |

**Foreign Keys:**
- `order_id` references `orders(order_id)` ON DELETE RESTRICT
- `repurchase_id` references `orders(order_id)` ON DELETE SET NULL

**Indexes:**
- `idx_order_id` on `order_id`
- `idx_status` on `status`
- `idx_repurchase_id` on `repurchase_id`
- `idx_created_at` on `created_at`

**Notes:**
- `repurchase_id` links to a new order that arose from the return process (exchange or promoted purchase)
- When null, indicates no repurchase was made from this return
